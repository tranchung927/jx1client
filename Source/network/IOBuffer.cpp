...
// #include "Exception.h"
// #include "Utils.h"
CIOBuffer::CIOBuffer(Allocator& allocator, unsigned int size)
    : m_operation(0), m_sequenceNumber(0), m_allocator(allocator), m_ref(1), m_size(size), m_used(0)
{
    //( ( BYTE * )( ( unsigned long )( pMemory + 3 ) & ( ~3 ) ) )
...
void CIOBuffer::operator delete(void* pObject, unsigned int /* bufferSize*/)
#else
...
CIOBuffer* CIOBuffer::SplitBuffer(unsigned int bytesToRemove)
{
...
VOID CIOBuffer::RemoveBuffer(unsigned int bytesToRemove)
{
...
void CIOBuffer::AddData(const char* const pData, unsigned int dataLength)
{
...
void CIOBuffer::AddData(const BYTE* const pData, unsigned int dataLength)
{
...
CIOBuffer::Allocator::Allocator(unsigned int bufferSize, unsigned int maxFreeBuffers)
...
CIOBuffer::InOrderBufferList::InOrderBufferList(CCriticalSection& criticalSection)
