...
class kuiCurlDown : public Ref {
private:
    std::string mFileName; // 下载文件名称
    bool isStop;
public:
    ~kuiCurlDown();
    kuiCurlDown();

    kuiCurlDown(std::string downUrl,std::string filePath);

    std::string mFilePath; // 本地存储地址
    double mFileLenth; // 下载文件大小
    std::string mDownloadUrl; // 下载URL
...
public:
     ...
     */
    virtual void onError(kuiCurlDown::ErrorCode errorCode) {};
